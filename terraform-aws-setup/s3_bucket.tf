...
resource "aws_s3_bucket_server_side_encryption_configuration" "encryption" {
  for_each = local.s3_bucket
  bucket   = aws_s3_bucket.ciscomcd_s3_bucket[each.key].id
  rule {
...
resource "aws_s3_bucket_policy" "ciscomcd_s3_bucket_policy" {
  ...
  policy = jsonencode({
    Version = "2012-10-17"
    Id      = "ciscomcd-bucket-policy"
    Statement = [
      {
        Action   = "s3:GetBucketAcl",
        Effect   = "Allow",
        Resource = aws_s3_bucket.ciscomcd_s3_bucket[each.key].arn,
        Principal = {
      ...
      },
      {
        Action   = "s3:PutObject",
        Effect   = "Allow",
        Resource = "${aws_s3_bucket.ciscomcd_s3_bucket[each.key].arn}/AWSLogs/${data.aws_caller_identity.current.account_id}/*",
        Principal = {
          Service = "cloudtrail.amazonaws.com"
        },
        Condition = {
