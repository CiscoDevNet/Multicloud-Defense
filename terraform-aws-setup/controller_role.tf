    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "sts:ExternalId" = ciscomcd_external_id.iam_external_id.external_id
          }
        }
      }
    ]
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          ...
          aws_iam_role.ciscomcd_inventory_role.arn,
          aws_iam_role.ciscomcd_fw_role.arn
        ]
      },
      {
        Action = [
...
resource "aws_iam_role_policy" "s3_get_object" {
  ...
  role     = aws_iam_role.ciscomcd_controller_role.id
  depends_on = [
    aws_s3_bucket.ciscomcd_s3_bucket
  ]
    ...
    Statement = [
      {
        ...
        Effect = "Allow",
        Resource = [
          "${aws_s3_bucket.ciscomcd_s3_bucket[each.key].arn}/*"
        ]
      }
    ]
  })
